...
const CANVAS_ID = 'body-weight-trend-chart'

export default function WeightTrendChart({ points = [] }) {
  const windowWidth = Taro.getSystemInfoSync().windowWidth
  const width = Math.max(240, Math.min(335, windowWidth - 52))
  const hasData = points.some((point) => point.weightKg !== null && point.weightKg !== undefined)
  ...
  useEffect(() => {
    ...
    const values = points
      .filter((point) => point.weightKg !== null && point.weightKg !== undefined)
    ...
    points.forEach((point, index) => {
      if (point.weightKg === null || point.weightKg === undefined) return
      const x = xFor(index)
      const y = yFor(Number(point.weightKg))
      if (!started) {
        context.moveTo(x, y)
        started = true
      } else {
    ...
    points.forEach((point, index) => {
      if (!point.recorded || point.weightKg === null || point.weightKg === undefined) return
      context.setFillStyle('#2f7d4a')
      context.beginPath()
      context.arc(xFor(index), yFor(Number(point.weightKg)), 4, 0, Math.PI * 2)
      context.fill()
    })
  ...
  if (!hasData) {
    return <View className='weight-chart-empty'><Text>这个区间还没有体重数据</Text></View>
  }

  return (
    <View className='weight-chart-wrap'>
      <Canvas canvasId={CANVAS_ID} style={{ width: `${width}px`, height: '210px' }} />
      <View className='weight-chart-legend'>
        <View className='weight-chart-dot' />
        <Text>圆点为真实记录，连线区间为沿用体重</Text>
      </View>
    </View>
  )
}
